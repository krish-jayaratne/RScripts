...
if %ERRORLEVEL% NEQ 0  goto error_info
echo  bteq logon !svr!/!dbName!_USER,%usrPW% ^< %test_acc_scr%
bteq logon !svr!/!dbName!_USER,%usrPW% < %test_acc_scr%
if %ERRORLEVEL% NEQ 0  goto error_info
...
if /i !drop!==yes (
    echo DELETE DATABASE  %dbname%_USER;  >> %cre_user_scr%
    ...
    echo DROP USER %dbname%_USER;  >> %cre_user_scr%
	for /F "delims=/ tokens=1-5" %%i in ("!schemas!") do (
...
echo CREATE USER %dbname%_USER FROM %dbname% AS PERM = %childsize%  NO BEFORE JOURNAL NO AFTER JOURNAL PASSWORD=%usrPW% ;  >> %cre_user_scr%
...
echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, MACRO, PROCEDURE, STATISTICS, EXECUTE, EXECUTE PROCEDURE ON %dbname%_REPO TO %dbname%_USER;  >> %g_user_scr%
echo GRANT EXECUTE PROCEDURE ON %dbname%_REPO TO %dbname%_REPO WITH GRANT OPTION;  >> %g_user_scr%

echo GRANT SELECT ON DBC.TABLESV TO %dbname%_USER;  >> %g_user_scr%
echo GRANT SELECT ON DBC.COLUMNSV TO  %dbname%_USER;  >> %g_user_scr%
echo GRANT SELECT ON SYSLIB.SQLRESTRICTEDWORDS TO %dbname%_USER;  >> %g_user_scr%
echo GRANT SELECT ON DBC.ErrorMsgs TO %dbname%_USER;  >> %g_user_scr%
...
for /F "delims=/ tokens=1-5" %%i in ("!schemas1!") do (
    if NOT %%ia==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, STATISTICS ON %dbname%_%%i TO %dbname%_USER;  >> %g_user_scr%
    if NOT %%ja==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, STATISTICS ON %dbname%_%%j TO %dbname%_USER;  >> %g_user_scr%
    if NOT %%ka==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, STATISTICS ON %dbname%_%%k TO %dbname%_USER;  >> %g_user_scr%
    if NOT %%la==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, STATISTICS ON %dbname%_%%l TO %dbname%_USER;  >> %g_user_scr%
    if NOT %%ma==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, STATISTICS ON %dbname%_%%m TO %dbname%_USER;  >> %g_user_scr%
)
