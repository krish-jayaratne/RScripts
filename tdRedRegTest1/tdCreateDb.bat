...
if %ERRORLEVEL% NEQ 0  goto error_info
echo  bteq logon !svr!/!dbName!,%usrPW% ^< %test_acc_scr%
bteq logon !svr!/!dbName!,%usrPW% < %test_acc_scr%
if %ERRORLEVEL% NEQ 0  goto error_info
...
echo.
echo Usage:  
echo    %0 'SeverIP' 'schema/db Name' DSNName={DSN to use}  		  
...
Rem *******************************************************************
:create_scr_schema
if /i !drop!==yes echo delete database %dbname%; > %cre_user_scr%
if /i !drop!==yes echo drop database %dbname%; >> %cre_user_scr%
echo CREATE USER %dbname% FROM DBC AS PERM = 200000000  NO BEFORE JOURNAL NO AFTER JOURNAL PASSWORD=%usrPW% ;  >> %cre_user_scr%
...
if /i !drop!==yes (
	...
	for /F "delims=/ tokens=1-5" %%i in ("!schemas!") do (
		if NOT %%ia==a echo DROP USER %dbname%_%%i;  >> %cre_user_scr%
		if NOT %%ja==a echo DROP USER %dbname%_%%j;  >> %cre_user_scr%
		if NOT %%ka==a echo DROP USER %dbname%_%%k;  >> %cre_user_scr%
		if NOT %%la==a echo DROP USER %dbname%_%%l;  >> %cre_user_scr%
		if NOT %%ma==a echo DROP USER %dbname%_%%m;  >> %cre_user_scr%
	)
)
for /F "delims=/ tokens=1-5" %%i in ("!schemas!") do (
	if NOT %%ia==a echo CREATE USER %dbname%_%%i FROM DBC AS PERM = 200000000 NO BEFORE JOURNAL NO AFTER JOURNAL PASSWORD =%usrPW%;  >> %cre_user_scr%
	if NOT %%ja==a echo CREATE USER %dbname%_%%j FROM DBC AS PERM = 200000000 NO BEFORE JOURNAL NO AFTER JOURNAL PASSWORD =%usrPW%;  >> %cre_user_scr%
	if NOT %%ka==a echo CREATE USER %dbname%_%%k FROM DBC AS PERM = 200000000 NO BEFORE JOURNAL NO AFTER JOURNAL PASSWORD =%usrPW%;  >> %cre_user_scr%
	if NOT %%la==a echo CREATE USER %dbname%_%%l FROM DBC AS PERM = 200000000 NO BEFORE JOURNAL NO AFTER JOURNAL PASSWORD =%usrPW%;  >> %cre_user_scr%
	if NOT %%ma==a echo CREATE USER %dbname%_%%m FROM DBC AS PERM = 10000000 NO BEFORE JOURNAL NO AFTER JOURNAL PASSWORD =%usrPW%;  >> %cre_user_scr%
)
...
echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, MACRO, PROCEDURE, STATISTICS, EXECUTE, EXECUTE PROCEDURE ON %dbname% TO %dbname% with GRANT option;  >  %g_user_scr%

set Schemas1=%schemas%

for /F "delims=/ tokens=1-5" %%i in ("!schemas!") do (
    if NOT %%ia==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, MACRO, PROCEDURE, STATISTICS, EXECUTE, EXECUTE PROCEDURE ON %dbname%_%%i TO %dbname% with GRANT option;  >> %g_user_scr%
    if NOT %%ja==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, MACRO, PROCEDURE, STATISTICS, EXECUTE, EXECUTE PROCEDURE ON %dbname%_%%j TO %dbname% with GRANT option;  >> %g_user_scr%
    if NOT %%ka==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, MACRO, PROCEDURE, STATISTICS, EXECUTE, EXECUTE PROCEDURE ON %dbname%_%%k TO %dbname% with GRANT option;  >> %g_user_scr%
    if NOT %%la==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, MACRO, PROCEDURE, STATISTICS, EXECUTE, EXECUTE PROCEDURE ON %dbname%_%%l TO %dbname% with GRANT option;  >> %g_user_scr%
    if NOT %%ma==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, MACRO, PROCEDURE, STATISTICS, EXECUTE, EXECUTE PROCEDURE ON %dbname%_%%m TO %dbname% with GRANT option;  >> %g_user_scr%
)

for /F "delims=/ tokens=1-5" %%i in ("!schemas1!") do (
    if NOT %%ia==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, MACRO, PROCEDURE, STATISTICS, EXECUTE, EXECUTE PROCEDURE ON %dbname%_%%i TO %dbname%_%%i with GRANT option;  >> %g_user_scr%
    if NOT %%ja==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, MACRO, PROCEDURE, STATISTICS, EXECUTE, EXECUTE PROCEDURE ON %dbname%_%%j TO %dbname%_%%j with GRANT option;  >> %g_user_scr%
    if NOT %%ka==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, MACRO, PROCEDURE, STATISTICS, EXECUTE, EXECUTE PROCEDURE ON %dbname%_%%k TO %dbname%_%%k with GRANT option;  >> %g_user_scr%
    if NOT %%la==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, MACRO, PROCEDURE, STATISTICS, EXECUTE, EXECUTE PROCEDURE ON %dbname%_%%l TO %dbname%_%%l with GRANT option;  >> %g_user_scr%
    if NOT %%ma==a echo GRANT SELECT, INSERT, UPDATE, DELETE, TABLE, VIEW, SHOW, MACRO, PROCEDURE, STATISTICS, EXECUTE, EXECUTE PROCEDURE ON %dbname%_%%m TO %dbname% with GRANT option;  >> %g_user_scr%
)
