...
for /F "delims=/ tokens=1-5" %%i in ("!schemas!") do (
	if NOT %%ia==a echo CREATE USER %dbname%_%%i FROM DBC AS PERM = 40000000 NO BEFORE JOURNAL NO AFTER JOURNAL PASSWORD =%usrPW%;  >> %cre_user_scr%
	if NOT %%ja==a echo CREATE USER %dbname%_%%j FROM DBC AS PERM = 20000000 NO BEFORE JOURNAL NO AFTER JOURNAL PASSWORD =%usrPW%;  >> %cre_user_scr%
	if NOT %%ka==a echo CREATE USER %dbname%_%%k FROM DBC AS PERM = 20000000 NO BEFORE JOURNAL NO AFTER JOURNAL PASSWORD =%usrPW%;  >> %cre_user_scr%
	if NOT %%la==a echo CREATE USER %dbname%_%%l FROM DBC AS PERM = 10000000 NO BEFORE JOURNAL NO AFTER JOURNAL PASSWORD =%usrPW%;  >> %cre_user_scr%
